...
function SubSocket(channel, opts) {
  ...
  var setup = channel.then(function(ch) {
    return ch.assertQueue(opts.queueName || '', {
      exclusive: opts.exclusive || true, autoDelete: opts.autoDelete || true, durable: opts.persistent || false
    }).then(function(ok) {
...
function ReqSocket(channel, opts) {
  ...
  var setup = channel.then(function(ch) {
    return ch.assertQueue(opts.queueName || '', {exclusive: opts.exclusive ||  true, autoDelete: opts.autoDelete || true})
      .then(function(ok) {
